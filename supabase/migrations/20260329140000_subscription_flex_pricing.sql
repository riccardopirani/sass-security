-- Dynamic subscription: enum value + seats column + signup default flex
alter type public.subscription_plan add value if not exists 'flex';

-- Table is still cg_subscriptions here (renamed to security_cg_* in 20260330).
alter table public.cg_subscriptions
...
begin
  ...
  selected_plan := case
    when selected_plan_text in ('starter', 'pro', 'business', 'flex')
